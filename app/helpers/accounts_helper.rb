module AccountsHelper
  def profile_picture account, width = 40, height = 40
    image_path = account.image.present? ? account.image.url : "default-avatar.jpg"
    image_tag image_path, width: width, height: width, class: "thumb"
  end

  def profile_picture_post post, width = 40, height = 40
    image_path = post.account.image.present? ? post.account.image.url : "default-avatar.jpg"
    image_tag image_path, width: width, height: width, class: "thumb"
  end

  def profile_picture_current current, width = 40, height = 40
    image_path = current.account.image.present? ? current.account.image.url : "default-avatar.jpg"
    image_tag image_path, width: width, height: width, class: "thumb"
  end
